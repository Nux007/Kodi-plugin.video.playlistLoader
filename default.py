...
def AddDir(name, url, mode, iconimage='', logos='', index=-1, move=0, isFolder=True, IsPlayable=False, background=None, cacheMin='0', plot="", fanart=""):
	...
	if mode == 1 or mode == 2 or mode == 32:
		items += [(getLocaleString(30040), 'XBMC.RunPlugin({0}?index={1}&mode=40&move=0)'.format(sys.argv[0], index)),
		tvdb_item,
		(getLocaleString(30030), 'XBMC.RunPlugin({0}?index={1}&mode={2}&move=-1)'.format(sys.argv[0], index, listMode)),
...
if mode == 0:
	...
	if makeGroups:
		...
		if token is False:
			common.OKmsg("The TV Db", getLocaleString(30042))
		else:
			common.startTheTvDbScan(index, playlistsFile, token)
			xbmc.executebuiltin("XBMC.Container.Refresh()")
	else:
		common.OKmsg("The TV Db", getLocaleString(30043))

elif mode == 42:
	ok = common.removeTheTvDBData(index)
	if ok:
		common.OKmsg("The TV Db", getLocaleString(32034))
		xbmc.executebuiltin("XBMC.Container.Refresh()")
	else:
		common.OKmsg("The TV Db", getLocaleString(32035))
